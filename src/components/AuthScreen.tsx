...
import { Zap, Lock, Mail, User as UserIcon, CheckCircle, ShieldCheck, ArrowRight, Compass } from 'lucide-react';
    ...
    try {
      if (isSignUp) {
        ...
        try {
          ...
          // Fallback to API mock auth if Firebase config is demo key
        ...
        try {
          ...
          const u: User = {
            ...
            email: res.user.email || email,
            token: await res.user.getIdToken(),
            favorites: ['neon-roast', 'cyber-bistro'],
    ...
    <div className="min-h-screen bg-[#050505] text-[#e5e2e1] flex flex-col justify-between items-center p-4 sm:p-8 relative overflow-hidden">
      ...
      <div className="w-full max-w-md bg-[#131313]/90 backdrop-blur-2xl border-2 border-[#00dbe9]/50 rounded-2xl p-6 sm:p-8 shadow-[0_0_60px_rgba(0,219,233,0.35)] my-6 relative">
        ...
        <h1 className="font-headline font-bold text-2xl text-[#e5e2e1] mb-1">
          {isSignUp ? 'Create Operator Account' : 'Sign In to FindFast AI'}
        </h1>
        ...
        {error && (
          <div className="p-3 rounded-xl bg-red-500/20 border border-red-500/40 text-xs font-mono text-red-400 mb-4">
            {error}
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4 font-mono text-xs">
          ...
          >
            {loading ? 'AUTHENTICATING...' : isSignUp ? 'CREATE OPERATOR ID' : 'SIGN IN OPERATOR'}
          </button>
        </form>
      ...
      <div className="text-center font-mono text-[10px] text-[#849495] pb-2">
        <span>GPS LOCK ACTIVE • FIREBASE & GOOGLE MAPS INTEGRATION</span>
      </div>
    </div>
  );
};
